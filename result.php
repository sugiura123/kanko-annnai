...
<br>
    ...
    <p class="search-result"><?php echo $prefectures[$key]; ?>の観光スポットは
    <?php echo count($places[$key]); ?>
    件見つかりました。</p>
    <div class="media">

      <div class="media-left">
        <img src="<?php
          // for文で回す回数は count($places[$key]) 件回す
          // ターゲットの画像のファイル名を出すには)

        if (count($places[$key]) > 0 :)
          # code...

        for ($i=0; $i <count($places[$key]) ; $i++) { echo $places[$key][$i][name];}
        for ($i=0; $i <count($places[$key]) ; $i++) { echo $places[$key][$i][detail];}
        for ($i=0; $i <count($places[$key]) ; $i++) { echo $places[$key][$i][image];}

...
<br>
    ...
    <?php echo $prefectures[$key]; ?>の観光スポットは
    <?php if ($places[$key] > 0 ) {
          echo count($places[$key])."件見つかりました。";}
          elseif ($places[$key] == null ) {echo "見つかりませんでした。" ;}
          else {;};
          ?>
    <div class="media">

      <div class="media-left">
        <img src=<?php for ($i=0; $i <count($places[$key]) ; $i++) {
          if (count($places[$key]) > 0 ):
              echo $places[$key][$i]["image"];
          endif;
          };?>img {display: block;
